...
AVATAR_COLORS = ["#6200EE", "#006EFF", "#00D68F", "#F89B29", "#FF0F7B", "#06b6d4", "#8b5cf6"]
...
def seed_demo_data():
    with Session(engine) as db:
        ...
        demo_users = [
            ...
            ("nusrat", "Nusrat Jahan", "Expert in digital marketing", "#FF0F7B", 2, True),
        ...
        order = [("masud", 0), ("nusrat", 0), ("tanvir", 0), ("sadia", 0), ("masud", 1), ("nusrat", 1)]
        all_posts = []
        minute = 5
        for username, idx in order:
            u = users[username]
            content = posts_by_username[username][idx]
            p = Post(user_id=u.id, content=content, created_at=created.replace(hour=10, minute=minute))
            db.add(p)
            db.flush()
            all_posts.append(p)
            minute += 5
        ...
        # comments
        comments = [
            (users["nusrat"].id, all_posts[0].id, "Congrats Masud, this is amazing! 🎉"),
            (users["tanvir"].id, all_posts[0].id, "I would love to join. Let's grow together 🚀"),
        ...
        db.add(Notification(
            user_id=users["masud"].id, actor_id=users["nusrat"].id, type="like",
            message="Nusrat Jahan liked your post", created_at=created,
        ))
        db.add(Notification(
            user_id=users["masud"].id, actor_id=users["tanvir"].id, type="follow",
            message="Tanvir Ahmed started following you", created_at=created,
        ))
        ...
        for username, content, grad in stories:
            db.add(Story(user_id=users[username].id, content=content, gradient=grad, created_at=created))
        db.commit()
...
def post_dict(p: Post, author: User, liked: bool, like_count: int, comments: list, reaction: str = "", bookmarked: bool = False) -> dict:
    return {
        "id": p.id,
        "content": p.content,
        "gradient": p.gradient,
...
def build_comments(db: Session, post_id: int) -> list:
    comments = db.exec(select(Comment).where(Comment.post_id == post_id).order_by(Comment.created_at.asc())).all()
    result = []
    for c in comments:
...
def build_post(db: Session, p: Post, user: Optional[User]) -> dict:
    ...
    bookmarked = bool(user and db.exec(select(Bookmark).where(Bookmark.post_id == p.id, Bookmark.user_id == user.id)).first())
...
async def search(q: str = "", user: Optional[User] = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    results = []
    if q:
        users = db.exec(select(User).where(User.username.contains(q) | User.name.contains(q))).all()
        posts = db.exec(select(Post)).all()
        matched = [p for p in posts if q in p.content.lower()]
        results = [
            {"type": "user", **public_user(u)}
            for u in users
        ] + [
            {"type": "post", **build_post(db, p, user)}
            for p in matched
        ]
...
@app.post("/api/posts")
async def create_post(body: dict = Body(...), user: User = Depends(require_user), db: Session = Depends(get_db)):
    content = (body.get("content") or "").strip()
    if not content:
        raise HTTPException(status_code=400, detail="Post content required")
    gradient = body.get("gradient") or ""
    post = Post(user_id=user.id, content=content, gradient=gradient if gradient else "linear-gradient(135deg,#6200EE,#D397FA)")
    db.add(post)
    db.commit()
    db.refresh(post)
...
async def toggle_like(post_id: int, body: dict = Body(default={}), user: User = Depends(require_user), db: Session = Depends(get_db)):
    post = db.get(Post, post_id)
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
    reaction = (body.get("reaction") or "like") if body else "like"
    existing = db.exec(select(Like).where(Like.post_id == post_id, Like.user_id == user.id)).first()
    liked = True
    if existing:
        ...
        if author and author.id != user.id:
            notify(db, author.id, user.id, "like", f"{user.name or user.username} reacted {reaction} to your post")
    db.commit()
...
async def toggle_bookmark(post_id: int, user: User = Depends(require_user), db: Session = Depends(get_db)):
    post = db.get(Post, post_id)
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
    existing = db.exec(select(Bookmark).where(Bookmark.post_id == post_id, Bookmark.user_id == user.id)).first()
    saved = True
    if existing:
        db.delete(existing)
        saved = False
...
async def create_story(body: dict = Body(...), user: User = Depends(require_user), db: Session = Depends(get_db)):
    content = (body.get("content") or "").strip()
    if not content:
...
async def add_comment(post_id: int, body: dict = Body(...), user: User = Depends(require_user), db: Session = Depends(get_db)):
    post = db.get(Post, post_id)
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
    content = (body.get("content") or "").strip()
    ...
    author = db.get(User, post.user_id)
    if author and author.id != user.id:
        notify(db, author.id, user.id, "comment", f"{user.name or user.username} commented on your post")
    db.commit()
    db.refresh(comment)
    return {"id": comment.id, "content": comment.content, "time": comment.created_at.strftime("%Y-%m-%d %H:%M"), "author": public_user(user)}
...
async def list_users(user: User = Depends(require_user), db: Session = Depends(get_db)):
    ...
    following_ids = {f.following_id for f in db.exec(select(Follow).where(Follow.follower_id == user.id)).all()}
    result = []
    for u in users:
        data = public_user(u)
        data["following"] = u.id in following_ids
        data["post_count"] = len(db.exec(select(Post).where(Post.user_id == u.id)).all())
        result.append(data)
    return {"users": result}


@app.post("/api/users/{user_id}/follow")
async def toggle_follow(user_id: int, user: User = Depends(require_user), db: Session = Depends(get_db)):
    target = db.get(User, user_id)
    if not target:
        raise HTTPException(status_code=404, detail="User not found")
    if target.id == user.id:
        raise HTTPException(status_code=400, detail="Cannot follow yourself")
    existing = db.exec(select(Follow).where(Follow.follower_id == user.id, Follow.following_id == user_id)).first()
    following = True
    if existing:
        db.delete(existing)
        following = False
        ...
        notify(db, user_id, user.id, "follow", f"{user.name or user.username} started following you")
    db.commit()
    return {"following": following}


@app.get("/api/connections")
async def connections(user: User = Depends(require_user), db: Session = Depends(get_db)):
    following_ids = [f.following_id for f in db.exec(select(Follow).where(Follow.follower_id == user.id)).all()]
    people = []
...
async def bookmarks(user: User = Depends(require_user), db: Session = Depends(get_db)):
    marks = db.exec(select(Bookmark).where(Bookmark.user_id == user.id).order_by(Bookmark.created_at.desc())).all()
    posts = []
    for m in marks:
...
async def get_notifications(user: User = Depends(require_user), db: Session = Depends(get_db)):
    notifs = db.exec(select(Notification).where(Notification.user_id == user.id).order_by(Notification.created_at.desc())).all()
    result = []
    for n in notifs:
...
async def user_posts(user_id: int, user: Optional[User] = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not target:
        raise HTTPException(status_code=404, detail="User not found")
    posts = db.exec(select(Post).where(Post.user_id == user_id).order_by(Post.created_at.desc())).all()
    return {"user": public_user(target), "posts": [build_post(db, p, user) for p in posts]}


@app.post("/api/profile")
async def update_profile(body: dict = Body(...), user: User = Depends(require_user), db: Session = Depends(get_db)):
    if "name" in body:
